    ...
    with log_filepath.open("wb") as fh:
        ...
        while total_size < max_size:
            timestamp = (
                ...
                f"{current_time.hour}:{current_time.minute:02}:{current_time.second:02}".encode(
                    "utf-8"
                )
            )

            message = (
                f"{person_name[int(6 * rand())]} "
            ...
            data = (
                message_template.replace(b"<LOG_LEVEL>", log_levels[int(4 * rand())])
